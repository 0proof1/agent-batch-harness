...
<p align="center"><strong>긴 에이전트 작업을 나누고, 각 shard를 검증하고, 맥락 손실 없이 이어서 실행합니다.</strong></p>
...
Shardflow는 큰 작업을 여러 shard로 나누어 실행하고 중단된 지점부터 다시
...
Shardflow는 이를 단순한 운영 규칙으로 바꿉니다.
...
```bash
python -m pip install path/to/shardflow-0.1.0-py3-none-any.whl
```
...
Shardflow 자체에는 Python 표준 라이브러리 이외의 runtime 의존성이 없습니다.
...
> Shardflow가 실행하는 agent CLI는 외부 의존성입니다. 폐쇄망에서는 wheel과
...
`items.tsv`는 실제 작업 단위의 source of truth입니다.
...
`shardflow plan`은 `_batches/manifest.tsv`를 만듭니다.
...
3. `shardflow plan`으로 manifest를 만듭니다.
4. `shardflow build-prompts`로 prompt를 생성합니다.
5. `shardflow resume`으로 다음 shard를 확인합니다.
